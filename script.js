...
const historyBtn = document.getElementById('historyNavBtn');
    ...
    // ربط زر الرئيسية لإعادة التعيين
    ...
    if(searchInput) {
        let searchTimeout; // متغير لتخزين المؤقت

        searchInput.addEventListener('input', (e) => {
            const term = e.target.value;
            
            // 1. إلغاء أي مؤقت سابق (إلغاء البحث القديم إذا كان المستخدم لا يزال يكتب)
            clearTimeout(searchTimeout);

            // 2. إنشاء مؤقت جديد ينتظر 600 جزء من الثانية (أكثر من نصف ثانية بقليل)
            searchTimeout = setTimeout(() => {
                
                // شرط الأداء: لا تبحث إلا إذا كان الحقل فارغاً (لإعادة التعيين) 
                // أو إذا كان النص يحتوي على حرفين أو أكثر
                if (term.length === 0 || term.trim().length >= 2) {
                    performSearch(term);
                }
                
            }, 800); // <--- هنا تتحكم في مدة المهلة (800ms ممتازة)
        });
    }
...
// ==========================================
// منطق نافذة المحفوظات (Favorites Modal Logic)
// ==========================================
...
// لاحظ إضافة المعامل الثاني: highlightTerm
function renderResults(data, highlightTerm = '') {
    ...
    data.forEach((item, index) => {
        ...
        // --- قسم التلوين ---
        // نقوم بتلوين العنوان والملخص بناءً على كلمة البحث
...
async function toggleFavorite(itemId) {
    ...
    // 1. تحديث قاعدة البيانات والمصفوفة المحلية
    if (userFavorites.has(itemIdStr)) {
        userFavorites.delete(itemIdStr); // حذف محلي
        try {
            await remove(favRef); // حذف من القاعدة
    ...
    // 2. تحديث الواجهة (DOM) مباشرةً بدلاً من إعادة الرسم (يمنع الوميض)
    ...
    // تحديث رقم العداد في البروفايل فقط
    updateStatsUI();
}
...
async function handleImageUpload(e) {
    ...
    if (!file || !currentUser) return;
       ...
       try {
        ...
        window.showToast("تم تحديث الصورة الشخصية", "success"); // <--- إضافة
    } catch (err) {
        window.showToast("فشل رفع الصورة", "error"); // <--- إضافة
    }
...
function setupTheme() {
    ...
    if(themeToggle) {
        themeToggle.addEventListener('click', () => {
            // 1. إيقاف الانتقالات مؤقتاً لمنع البطء والتشوه
            ...
            // 4. إعادة تفعيل الانتقالات بعد أن ينتهي المتصفح من تغيير الألوان
            // نستخدم setTimeout للسماح للمتصفح برسم الصفحة الجديدة أولاً
            setTimeout(() => {
                html.classList.remove('disable-transitions');
            }, 50); // 50 ميلي ثانية كافية جداً
        });
    }
}

// ==========================================
// منطق البحث المحسن (Smart Search)
// ==========================================

// دالة لتوحيد النصوص العربية (إزالة التشكيل وتوحيد الحروف المتشابهة)
function normalizeArabic(text) {
    if (!text) return "";
    text = String(text);
    
    // 1. إزالة التشكيل (الفتحة، الضمة، الكسرة، السكون، الشدة، التنوين)
    ...
    // ملاحظة: يتم تحويل "ئ" إلى "ي" لتسهيل البحث
    text = text.replace(/[ىئ]/g, "ي");
    
    // 5. توحيد الواو والهمزة على واو (ؤ -> و)
    text = text.replace(/ؤ/g, "و");
    
    // 6. تجاهل الهمزة المنفردة (ء) - اختياري، يسهل البحث
    text = text.replace(/ء/g, "");

    // 7. معالجة الحروف المتشابهة صوتياً (ظ <-> ض)
    // نقوم بتحويل "ظ" إلى "ض" في النصين لضمان التطابق مهما كتب المستخدم
...
function performSearch(term) {
    ...
    // 1. تنظيف النص وتقسيمه إلى كلمات (Array of words)
    ...
    const filtered = allData.filter(item => {
        ...
        // تجهيز نصوص العنصر للمقارنة
        const normalizedTitle = normalizeArabic(item.title);
        const normalizedDetails = normalizeArabic(item.details);
        const itemId = item.id.toString();

        // 2. منطق البحث المرن (AND Logic)
        // يجب أن تكون *كل* كلمة كتبها المستخدم موجودة في (العنوان أو التفاصيل أو الرقم)
    ...
    // نمرر النص الأصلي (rawTerm) لدالة العرض ليتم تلوين الكلمات
...
// ==========================================
// تحديث نهائي: نسخ ومشاركة السؤال + الجواب + التوقيع
// ==========================================

window.copyContent = async function(btnElement) {
    // 1. تحديد الزر (Fallback)
    let btn = btnElement;
    ...
    // 3. تجهيز النص الكامل مع التوقيع
    ...
    try {
        ...
        try {
            ...
            if (successful) {
                showSuccessEffect(btn);
        window.showToast("تم نسخ النص للحافظة", "success"); // <--- إضافة (اختياري، لأن الزر يتغير شكله أيضاً)
...
window.shareContent = function(btnElement) {
    ...
    // تجهيز النص الكامل مع التوقيع للمشاركة
    const appSignature = "\n\n---\nتمت المشاركة بواسطة تطبيق منهج النور";
    const fullText = `السؤال:\n${titleElement.innerText}\n\nالجواب:\n${textElement.innerText}${appSignature}`;
    ...
    const shareData = {
        ...
        text: fullText, 
        url: window.location.href, // رابط الصفحة سيظهر أيضاً في معظم التطبيقات
    };
...
async function openHistoryModal() {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            // تحويل الكائن لمصفوفة مع الاحتفاظ بالـ ID
...
async function addNote() {
    ...
    // إضافة تأثير تحميل بسيط على الزر (اختياري)
    input.value = ''; // تفريغ الحقل فوراً لتجربة مستخدم سريعة
    ...
    try {
        ...
        // إعادة تحميل القائمة لتظهر الملاحظة الجديدة
        const snapshot = await get(ref(db, `users/${currentUser.uid}/notes`));
        if (snapshot.exists()) {
...
async function deleteNote(noteId) {
    ...
    try {
        ...
        // إخفاء العنصر من الواجهة مباشرة
...
window.showToast = function(message, type = 'success') {
    ...
    // إظهار الإشعار (Animation In)
    requestAnimationFrame(() => {
        toast.classList.remove('translate-y-[-20px]', 'opacity-0');
    });

    // إخفاء الإشعار بعد 3 ثواني (Animation Out)
    setTimeout(() => {
        toast.classList.add('opacity-0', 'translate-y-[-20px]');
        // إزالة العنصر من الصفحة تماماً بعد انتهاء الأنيميشن
        setTimeout(() => toast.remove(), 300);
    }, 3000);
};
// دالة لتلوين النص المطابق للبحث مع مراعاة التشكيل والهمزات
// دالة لتلوين النص المطابق للبحث (تدعم كلمات متعددة متفرقة)
function highlightText(text, searchTerm) {
    if (!searchTerm || searchTerm.trim().length < 2) return text;

    // 1. تقسيم نص البحث إلى كلمات منفصلة
    const terms = searchTerm.trim().split(/\s+/).filter(w => w.length > 0);
    
    // 2. بناء نمط Regex مجمع لكل الكلمات (Word1|Word2|Word3)
    let patternParts = [];
    ...
    try {
        // نجمع الأنماط بـ OR (|) لتلوين أي كلمة منها تظهر في النص
